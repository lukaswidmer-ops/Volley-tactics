...
function paintLobby(room) {
  ...
  if (!app) return;
  setAppView('mp_lobby');
  // game.js state.view mit DOM synchron halten (sonst applyRemoteState mit prevView vs. targetView falsch)
  try {
    if (window.VV && window.VV.state && window.VV.state.view !== 'mp_lobby' && typeof window.VV.setView === 'function') {
      window.VV.setView('mp_lobby');
    }
  } catch (_) {}
  const meta = (room && room.meta) || {};
  const playersMap = (room && room.players) || {};
